...
const TeamsHeatmapDashboard = () => {
  ...
  useEffect(() => {
    const fetchTeamsHeatmap = async () => {
      setLoading(true);
      setError(null);
      
      try {
        // Get all employees for manager's teams
        ...
        // Fetch teams heatmap using team endpoint
        const heatmapData = await analyticsAPI.getTeamHeatmap('manager-teams', employeeIds);
        
        // Transform members to include employee info
        const membersWithInfo = employees.map((emp) => ({
          id: emp.personal_number,
          name: emp.name,
          department: 'General', // Could be enhanced with employee details
          position: 'Employee', // Could be enhanced with employee details
          userName: emp.name,
        }));
        ...
        setError(err.message || 'Failed to fetch teams heatmap');
        console.error('Error fetching teams heatmap:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Box>
      ...
      <Typography 
        ...
        sx={{ mb: 4 }}
      >
        Skills overview across all team members
      </Typography>
                ...
                <TableRow key={member.id} hover>
                  ...
                  {skills.map((skill, skillIndex) => {
                    ...
                    return (
                      <Tooltip
                        key={skillIndex}
                        title={`${member.name}: ${skill} - Level ${value}/5`}
                        arrow
                      >
                        <TableCell
                          align="center"
...
export default TeamsHeatmapDashboard;
